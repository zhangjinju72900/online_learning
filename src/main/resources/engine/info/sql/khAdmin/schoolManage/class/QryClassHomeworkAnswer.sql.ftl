select * from (
	...
	case when a.status = 3 and a.homework_type = 1 then d1.name
	when a.status = 3 and a.homework_type = 0 then a.objective_real_score
	...
	where a.homework_id =  #{data.homeworkId} and a.valid_flag = 0 

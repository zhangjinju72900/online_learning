select * from (
	SELECT
       ...
       case il.like_by when #{data.userId} then '已点赞'
	   else '未点赞'
	   end as likeValidName,
	   case il.like_by when #{data.userId} then '1'
	   else '0'
	   end as likeValid
	   from
	...
	where ir.valid_flag = 0 and ir.check_status = 1 and ir.review_id = #{data.id}
		order by ir.top_flag desc ,ir.top_time desc, check_time desc, ir.id desc
		limit 0,5
) a 

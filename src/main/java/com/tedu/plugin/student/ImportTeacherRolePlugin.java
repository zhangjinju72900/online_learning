 ...
 *
 */
@Service("importTeacherRolePlugin")
public class ImportTeacherRolePlugin implements ILogicServicePlugin {
	@Resource
	...
	public void doAfter(FormEngineRequest requestObj, FormModel formModel, Object beforeResult) {
		...
		String classNames = map.get("className")==null?"":map.get("className").toString();
		String[] clNames=StringUtils.isNotBlank(classNames)?classNames.startsWith("[")?classNames.substring(1, classNames.length()-1).split(","):classNames.split(","):null;
		
		String gradeNames = map.get("gradeName")==null?"":map.get("gradeName").toString();
		String[] geNames=StringUtils.isNotBlank(gradeNames)?gradeNames.startsWith("[")?gradeNames.substring(1, gradeNames.length()-1).split(","):gradeNames.split(","):null;
		if(clNames != null && geNames != null){
			for (int i=0; i<clNames.length; i++) {
				qlist = new ArrayList<Map<String, Object>>();
				qp = new QueryPage();
				map1 = new HashMap<>();
				map1.put("schoolId", schoolId);
				map1.put("name", clNames[i]);
				map1.put("gradeName", geNames.length > i ? geNames[i] : "");
				qp.setDataByMap(map1);
				qp.setQueryParam("QryCustomUserClassId");
				qlist = formService.queryBySqlId(qp);
				if(qlist != null && qlist.size() > 0){
					
					cModel = new CustomFormModel();
					cModel.setSqlId("insertCustomUserClass");
					cModel.setData(formModel.getData());
					logData.put("userId", userId);
					logData.put("classId", qlist.get(0).get("id"));
					formMapper.saveCustom(cModel);

 ...
 *
 */
@Service("importPPTExecute")
public class ImportPPTExecute implements ILogicPlugin {
	...
	public void doAfter(FormEngineRequest requestObj, FormModel formModel, Object beforeResult,
		...
		try {
			...
			ossRecordServiceImpl.insertOssRecord(fileName, fileType, fileId, "", BUCKET_NAME1);*/
			...
			customResourcesServiceImpl.insertResources(new ResourcesFile(fileName, path, fileType, fullPath), "", Long.parseLong(id), fileId, 
					"", "1.0", ResourceTypeEnum.PPT.getCode(), CustomerResourcesSourceEnum.TFILEINDEX.getCode());
		} catch (Exception e) {
			log.error("上传资源失败！", e);
			e.printStackTrace();

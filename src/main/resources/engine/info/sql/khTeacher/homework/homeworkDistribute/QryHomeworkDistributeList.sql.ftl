select * from (
	...
	'待批改' as distributeStatusName,
	cl.name as className,
	co.name as courseName,
	se.name as sectionName,
	case when NOW()>end_time then 0 else 1 end correctStatus
	from t_homework h
	...
	where h.teacher_id = #{data.session.userInfo.userId} and h.valid_flag = 0 and h.distribute_status='1'

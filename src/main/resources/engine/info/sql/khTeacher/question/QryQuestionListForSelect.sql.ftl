...
where tcu.id = #{data.session.userInfo.userId} and tc.valid_flag = 0 and tp.valid_flag = 0
	...
	where q.valid_flag = 0 and q.question_type!=3 and (q.data_flag = 0 or teacher_id = #{data.session.userInfo.userId}) and q.data_flag in(0,1)  order by q.create_time desc 
 ) a 
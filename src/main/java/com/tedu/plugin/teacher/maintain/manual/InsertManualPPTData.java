...
/**
 * 维修手册PPT转H5zip文件新建/编辑处理
 * @author qun
 *
 */
@Service("insertManualPPTData")
public class InsertManualPPTData implements ILogicPlugin {
    ...
    public void doAfter(FormEngineRequest requestObj, FormModel formModel, Object beforeResult, FormEngineResponse responseObj) {
		...
		try {
			...
			if(map.get("id") != null && StringUtils.isNotBlank(map.get("id").toString())){//编辑
				...
				updateMap.put("ossKey", "");
				updateMap.put("oss_url", "");
				cModel.setData(updateMap);
	        	cModel.setSqlId("khAdmin/maintainManual/updateManual");
	        	formMapper.saveCustom(cModel);
			}else{//新增
				maintainManualServiceimpl.insertMaintainManual(new ResourcesFile(fileName, path, fileType, fullPath), "", Long.parseLong(parentId), fileId, "", 
						ResourceTypeEnum.PPT.getCode(), CustomerResourcesSourceEnum.TFILEINDEX.getCode());
			}
		} catch (Exception e) {
			log.error("上传资源失败！", e);
			e.printStackTrace();

SELECT
	...
	CASE when ha.status = 3 then 
		CASE when ha.homework_type = 0 then FORMAT(objective_real_score,0)
			else d.`name` end 
	else '无'  end  answerResult,
...
and ha.valid_flag = 0 and ha.student_id = #{data.studentId} and DATE_FORMAT(h.distribute_time,'%Y-%m-%d') = #{data.distributeTime}
